...
function validateConfig() {
    const errors = [];
    if (!exports.config.wallet.secretKey) {
        errors.push('BOT_SECRET_KEY is required');
    }
    if (!exports.config.contracts.flashLoanExecutor || exports.config.contracts.flashLoanExecutor.startsWith('CX')) {
        errors.push('FLASH_LOAN_EXECUTOR_CONTRACT must be set to a valid contract address');
    }
    if (exports.config.trading.minProfitBps < 0 || exports.config.trading.minProfitBps > 10000) {
        errors.push('MIN_PROFIT_BPS must be between 0 and 10000');
    }
    if (exports.config.trading.maxSlippageBps < 0 || exports.config.trading.maxSlippageBps > 10000) {
